...
/**
 * digitalocean GPT engine implementation
 *
 * Supports GPT-4, Llama 3 70B, GPT-3.5-turbo and o1 models via digitalocean API
 */
class digitaloceanModel implements AIModelEngineInterface
{
    ...
    public function __construct(
        ...
        $defaultConfig = config('ai.engines.digitalocean', []);
    ...
    {
        return config('ai.engines.digitalocean.display_name', 'digitalocean GPT');
    }
    ...
    {
        return config(
            'ai.engines.digitalocean.description',
            'digitalocean GPT модели - самые популярные и мощные языковые модели. Включает Llama 3 70B, Llama 3 70B, Llama 3 8B и модели рассуждений o1. Отличается высоким качеством и надежностью.'
        );
    }
    ...
    {
        $models = config('ai.engines.digitalocean.models', []);
        $validation = config('ai.engines.digitalocean.validation', []);
        ...
        return [
            'api_key' => [
                'type' => 'password',
                'label' => 'API key',
                'description' => 'API key from digitalocean Platform',
                'placeholder' => 'sk-...',
                'required' => true
            ],
            'model' => [
                'type' => 'select',
                'label' => 'digitalocean Model',
                'description' => 'Select an digitalocean model to use',
    ...
    public function getAvailableModels(?array $config = null): array
    {
        ...
        // digitalocean requires API key for model listing
        if (empty($apiKey)) {
            $this->logger->info('No digitalocean API key provided, using fallback models');
            return $this->getFallbackModels();
        }

        $cacheKey = 'digitalocean_models_' . substr(md5($apiKey), 0, 8);
        $cacheLifetime = config('ai.engines.digitalocean.models_cache_lifetime', 3600); // 1 hour

        return $this->cache->remember($cacheKey, $cacheLifetime, function () use ($apiKey) {
            try {
                $modelsEndpoint = config('ai.engines.digitalocean.models_endpoint', 'https://api.digitalocean.com/v1/models');
                ...
                $timeout = config('ai.engines.digitalocean.timeout', 30);

                $this->logger->info('Fetching digitalocean models from API', [
                ...
                if ($response->successful()) {
                    ...
                    foreach ($data['data'] ?? [] as $model) {
                        ...
                        $models[$modelId] = [
                            ...
                            'owned_by' => $model['owned_by'] ?? 'digitalocean',
                    ...
                    if (!empty($models)) {
                        $this->logger->info('digitalocean models loaded from API', [
                            'count' => count($models)
                        ]);
                        return $models;
                    }
                ...
                $this->logger->warning('Failed to fetch digitalocean models from API', [
                    'status_code' => $response->status(),
                    'response_body' => $response->body()
                ]);

            } catch (\Exception $e) {
                $this->logger->error('Error fetching digitalocean models: ' . $e->getMessage(), [
            ...
            $this->logger->info('Using fallback models for digitalocean after API failure', ['count' => count($fallbackModels)]);
    ...
    {
        $contextLengths = [
            'meta/llama-3-70b-instruct' => 128000,
            'meta/llama-3-8b-instruct' => 128000,
            'meta/llama-3-70b-instruct' => 128000,
            'gpt-4' => 8192,
            'meta/llama-3-8b-instruct' => 16385,
            'meta/llama-3-70b-instruct' => 128000,
            'o1-mini' => 128000
        ];
    ...
    protected function formatModelDisplayName(string $modelId): string
    {
        ...
        $specialNames = [
            ...
            'meta/llama-3-8b-instruct' => 'Llama 3 70B Mini',
            'meta/llama-3-70b-instruct' => 'Llama 3 70B',
            'gpt-4' => 'GPT-4',
            'meta/llama-3-8b-instruct' => 'Llama 3 8B',
            'meta/llama-3-70b-instruct' => 'o1 Preview',
            'o1-mini' => 'o1 Mini'
        ];
    ...
    {
        $descriptions = [
            'meta/llama-3-70b-instruct' => 'Latest GPT-4 Omni model with multimodal capabilities',
            'meta/llama-3-8b-instruct' => 'Smaller, faster version of Llama 3 70B',
            'meta/llama-3-70b-instruct' => 'Latest Llama 3 70B model with improved performance',
            'gpt-4' => 'Original GPT-4 model with advanced reasoning',
            'meta/llama-3-8b-instruct' => 'Fast and cost-effective model for most tasks',
            'meta/llama-3-70b-instruct' => 'Advanced reasoning model optimized for complex problems',
    ...
    public function getRecommendedPresets(): array
    {
        ...
        $configPresets = config('ai.engines.digitalocean.recommended_presets', []);
    ...
    protected function getDefaultPresets(): array
    {
        return [
            [
                'name' => 'Llama 3 70B - Creative',
                'description' => 'Creative Writing and Brainstorming with Llama 3 70B',
                'config' => [
            ...
            [
                'name' => 'Llama 3 70B - Accurate',
                'description' => 'Accurate and factual answers with Llama 3 70B',
                'config' => [
                    'model' => 'meta/llama-3-70b-instruct',
                    'temperature' => 0.2,
                    'top_p' => 0.8,
    ...
    {
        return [
            'model' => config('ai.engines.digitalocean', 'meta/llama-3-70b-instruct'),
            'max_tokens' => (int) config('ai.engines.digitalocean.max_tokens', 4096),
            'temperature' => (float) config('ai.engines.digitalocean.temperature', 0.8),
            'top_p' => (float) config('ai.engines.digitalocean.top_p', 0.9),
            'frequency_penalty' => (float) config('ai.engines.digitalocean.frequency_penalty', 0.0),
            'presence_penalty' => (float) config('ai.engines.digitalocean.presence_penalty', 0.0),
            'api_key' => config('ai.engines.digitalocean.api_key', ''),
            'server_url' => config('ai.engines.digitalocean.server_url', 'https://api.digitalocean.com/v1/inference'),
            'system_prompt' => config('ai.engines.digitalocean.system_prompt', 'You are a useful AI assistant.')
        ];
    }
    ...
    public function validateConfig(array $config): array
    {
        ...
        $supportedModels = array_keys(config('ai.engines.digitalocean.models', []));
    ...
    public function testConnection(): bool
    {
        try {
            ...
            $timeout = config('ai.engines.digitalocean.timeout', 10);
     ...
     *
     * digitalocean is the canonical tool_calls provider — the format used by all
     * other compatible engines originates here.
     *
     * Two response paths when tool schemas are attached:
    ...
    public function generate(AiModelRequestInterface $request): AiModelResponseInterface
    {
        try {
            ...
            $headers = $this->getRequestHeaders();
            $timeout = config('ai.engines.digitalocean.timeout', 120);
            ...
            if ($response->failed()) {
                ...
                throw new AiModelException("digitalocean API Error ({$response->status()}, {$errorCode}): $errorMessage");
            }
            ...
            if (isset($result['usage']) && config('ai.engines.digitalocean.log_usage', true)) {
                $this->logger->info('digitalocean tokens used', [
                    'model'             => $this->model,
            ...
            if ($finishReason === 'tool_calls' && !empty($message['tool_calls'])) {
                $this->logger->info('digitalocean tool_calls received', [
                    'model' => $this->model,
            ...
            if (!isset($message['content'])) {
                $this->logger->warning('Invalid digitalocean response format', ['response' => $result]);
                throw new AiModelException(
                    config('ai.global.error_messages.invalid_format', 'Invalid response format from digitalocean API')
                );
            }
            ...
            $this->logger->error('digitaloceanModel error: ' . $e->getMessage(), [
    ...
    /**
     * Handle different types of digitalocean errors
     */
    protected function handleError(\Exception $e): ModelResponseDTO
    {
        $errorMessages = config('ai.engines.digitalocean.error_messages', []);

        // Check for specific error types
        if (str_contains($e->getMessage(), 'rate_limit_exceeded')) {
            $message = $errorMessages['rate_limit'] ?? "digitalocean request limit exceeded. Please try again later.";
            return new ModelResponseDTO("error\n{$message}", true);
        }

        if (str_contains($e->getMessage(), 'insufficient_quota')) {
            $message = $errorMessages['insufficient_quota'] ?? "Not enough digitalocean quota. Check your account balance.";
            return new ModelResponseDTO("error\n{$message}", true);
        }

        if (str_contains($e->getMessage(), 'API Error')) {
            $message = $errorMessages['api_error'] ?? "Error from digitalocean API: " . $e->getMessage();
            return new ModelResponseDTO("error\n{$message}", true);
        }

        $message = $errorMessages['general'] ?? "Error contacting digitalocean: " . $e->getMessage();
        return new ModelResponseDTO("error\n{$message}", true);
    }
    ...
    {
        $baseHeaders = config('ai.engines.digitalocean.request_headers', [
            'Content-Type' => 'application/json',
        ]);
    ...
    /**
     * Build messages array for the digitalocean API.
     *
     * Role mapping:
     *   user    → role: user
     ...
     *             role: assistant (plain, tag-mode fallback)
     *   default → role: assistant
     *
     * @param  AiModelRequestInterface $request
     * @return array                   Messages array ready for the digitalocean API
     */
            ...
            switch ($role) {
                ...
                case 'command':
                    ...
                    if ($toolCallsRaw) {
                        ...
                        if (!empty($toolCalls)) {
                            // Emit null content — digitalocean spec requires content=null
    ...
    {
        if (empty($output)) {
            $errorMessage = config('ai.global.error_messages.empty_response', 'Error: digitalocean did not provide a response.');
        ...
        $cleanupPattern = config('ai.engines.digitalocean.cleanup.role_prefixes', '/^(Assistant|AI|GPT):\s*/i');
        $cleanOutput = preg_replace($cleanupPattern, '', $cleanOutput);

        if (empty($cleanOutput)) {
            $errorMessage = config('ai.global.error_messages.empty_response', 'Error: digitalocean returned an empty response.');
            return "response_from_model\n{$errorMessage}";
        }
    ...
    public function setMode(string $mode): void
    {
        ...
        $modePresets = config('ai.engines.digitalocean.mode_presets', []);
    ...
    public function isModelAvailable(): bool
    {
        try {
            ...
            $this->logger->warning("Failed to check digitalocean model availability: " . $e->getMessage());
            return false;
        }
    }
    ...
    protected function getFallbackModels(): array
    {
        return [
            ...
            'meta/llama-3-8b-instruct' => [
                ...
                'display_name' => 'Llama 3 70B Mini',
                'description' => 'Smaller, faster version of Llama 3 70B',
                'context_length' => 128000,
                'owned_by' => 'digitalocean',
                'category' => 'general',
                'recommended' => true
