...
- [ ] 1. Phase 1 — Backend: Critical Bug Fixes
  - [ ] 1.1 Fix `clone` PHP fatal error in `ContactImportController::importMessages()`
  ...
  - [ ] 1.2 Delete zero-byte misplaced route files
  ...
  - [ ] 1.3 Remove `DELETE /contacts/{id}/erase` route conflict
  ...
  - [ ] 1.4 Fix `ContactController::messages()` cache key to use parameter whitelist
  ...
  - [ ] 1.5 Fix `ContactStatsService` failed counts to read from database tables
  ...
  - [ ] 1.6 Add database migrations for missing columns
...
- [ ] 2. Phase 1 — Frontend: Critical Bug Fixes

  - [ ] 2.1 Fix `NxMessageViewer` — replace raw `fetch()` with `apiClient`
  ...
  - [ ] 2.2 Fix `NxRulesViewer` — replace mock data with real API calls
  ...
  - [ ] 2.3 Fix `NxAiAnalysisModal` — add `suggest_rules` checkbox and selectors
  ...
  - [ ] 2.4 Fix Contact360 `useEffect` tab switch — add `topics` and `audit` cases
  ...
  - [ ] 2.5 Fix `ContactHubTopbarControls` triple-nested `.data.data.data` accessor
...
- [ ] 4. Phase 2 — Backend: Async Import Pipeline — Services and Jobs

  - [ ] 4.1 Create `ContactImportPreviewService` standalone service
  ...
  - [ ] 4.2 Create `ContactImportRollbackService` standalone service
  ...
  - [ ] 4.3 Create async import queue jobs
  ...
  - [ ] 4.4 Update import controller endpoints to dispatch async jobs
...
- [ ] 5. Phase 3 — Backend: Authorization Policy and Hub Events

  - [ ] 5.1 Create `ContactPolicy` and register it
  ...
  - [ ] 5.2 Add rate limiting to import and analysis endpoints
  ...
  - [ ] 5.3 Create the 6 missing Laravel event classes
  ...
  - [ ] 5.4 Wire events in `EventServiceProvider` and dispatch event call-sites
...
- [ ] 8. Phase 5 — Backend: Memory Maintenance, Privacy Jobs

  - [ ] 8.1 Enable global scope in `ContactMemoryMaintenancePipeline`
  ...
  - [ ] 8.2 Create `ExportContactDataJob` and `EraseContactDataJob` privacy jobs
  ...
  - [ ] 8.3 Update export and erase controller endpoints to dispatch async jobs
  ...
  - [ ] 8.4 Create remaining maintenance queue job classes
...
- [ ] 9. Phase 6 — Frontend: New Contact360 Tabs

  - [ ] 9.1 Add WhatsApp and Facebook tabs to Contact360
  ...
  - [ ] 9.2 Create `NxConversationsViewer` component and add Conversations tab
  ...
  - [ ] 9.3 Create `NxMemoriesViewer` component and add Memories tab
  ...
  - [ ] 9.4 Create `NxIntelligencePanel` component and add Intelligence tab
  ...
  - [ ] 9.5 Create `NxAnalysisFindingsReview` component and wire into AI Analysis tab
...
- [ ] 10. Phase 7 — Frontend: Contact Cards, Topbar, and Import Modal

  - [ ] 10.1 Update `NxContactCard3D` with all required fields and 7 quick actions
    - Extend the component's props interface to include: `gender`, `tags`,
      `emotional_baseline`, `conflict_count`, `last_interaction_at`
    - Add new optional callback props: `onOpenProfile`, `onStartAnalysis`,
      `onImportMessages`, `onViewConversations`, `onEditReplyMode`, `onMerge`, `onArchive`
    - Render all data fields when present (WhatsApp number, contact type badge, gender
      badge, reply mode indicator, override indicator, last interaction, emotional
      baseline chip, profile confidence, memory freshness, conflict count)
    - Add a collapsible quick-actions row (appears on hover) with icon buttons for all
      7 actions
    - _Requirements: 15.1, 15.2_

  - [ ] 10.2 Wire `ContactHubTopbarControls` Maintain button to global maintenance modal
    - In `app/contacts/page.tsx`, add `isMaintenanceModalOpen` state and pass a real
      `onMaintenanceClick` handler to `ContactHubTopbarControls` that sets the state true
    - Render `<NxMemoryMaintenanceModal isOpen={...} onClose={...} scope="global" />`
      on the contacts page
    - The modal must render a scope selector (all contacts, stale only, conflicted only,
      selected contacts) as specified in Req 9.1
    - _Requirements: 9.1, 15.3_

  - [ ] 10.3 Wire `ContactHubTopbarControls` Import button to `NxImportModal`
    - In `app/contacts/page.tsx`, add `isImportModalOpen` state and pass a real
      `onImportClick` handler to `ContactHubTopbarControls` that opens `NxImportModal`
    - Replace the contacts page inline import drawer with the `NxImportModal` component
      (which has source selector, file upload, preview step, contact-matching step,
      progress tracking, and rollback)
    - _Requirements: 15.6, F-M-08_

  - [ ] 10.4 Extend queue/progress indicator to cover import and maintenance jobs
    - In the contacts page, extend the existing `pendingRuns > 0` indicator to also
      show counts of active import jobs (from `/contacts/stats`) and active maintenance
      jobs
    - The indicator should display import, analysis, and maintenance job counts separately
    - _Requirements: 15.5_
...
- [ ] 11. Phase 8 — Frontend: Topics Evidence and Relationship Graph

  - [ ] 11.1 Update `NxTopicsViewer` to expand topics with evidence citations
    - In `components/NxTopicsViewer.tsx` (or equivalent), add an `expandTopic` handler
      that calls `GET /api/v1/contacts/{contactId}/topics/{topicId}/mentions` via
      `apiClient` when a topic row is expanded
    - Render each mention as a `NxSourceCitation` showing: message excerpt, sender,
      timestamp, and analysis run ID (clickable)
    - If the topic has `analysis_run_id`, display a confidence badge and run ID link in
      the topic header
    - _Requirements: 16.2, 16.4_

  - [ ] 11.2 Install `react-force-graph-2d` and create `NxRelationshipGraph` component
    - Add `react-force-graph-2d` to `package.json` in `Nexus-Frontend/`
    - Create `components/NxRelationshipGraph.tsx` using a dynamic import with `ssr: false`
    - Props: `{ contactId: number; contactName: string; relationships: ContactRelationship[];
      onNodeClick: (relatedContactId: number) => void }`
    - Render nodes (current contact + related contacts) and edges encoded by relationship
      type (color) and strength (width)
    - On node click: route to `/contacts/${relatedId}` or show a `NxPopover` mini-profile
    - Empty state (< 2 relationships): render `NxEmptyState`
    - _Requirements: 17.1, 17.2, 17.3, 17.4_

  - [ ] 11.3 Add graph view toggle to Relationships tab in Contact360
    - In Contact360's Relationships tab, add a `[List view] | [Graph view]` toggle
    - When graph view is active, render `<NxRelationshipGraph />` using the already-loaded
      relationships data
    - When list view is active, render the existing flat list
    - _Requirements: 17.1_
